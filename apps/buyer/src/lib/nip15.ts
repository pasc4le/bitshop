...
export async function getCategoryForStall(stall: {
  stallEvtId: string;
  content: StallContent;
}): Promise<string> {
  ...
  await relay.connect().catch(() => {
    throw new Error("Relay connection failure");
  });
...
export async function getSampleImageForStall(
  stallEventId: string
): Promise<string> {
  const relay = relayInit(RELAY_EP);
  await relay.connect();
...
export async function lazyGetAllProductsForStall(
  stall_id: string,
  pubkey: string,
  onNew?: (p: ProductEvent) => void
) {
  ...
  return new Promise((resolve, reject) => {
    ...
    sub.on("event", (event: Event) => {
      const evt = {
        ...event,
        content: JSON.parse(event.content) as ProductEventContent,
      };
...
export async function getStallsByPubkey(
  pubkey: string,
  onNew?: (s: StallEvent) => void
) {
  const pool = new SimplePool();
  const filter: Filter = {
    authors: [pubkey],
    kinds: [EVENT_KINDS.PUBLISH_STALL],
    // kinds: [300170, 30017],
  };
  ...
  return new Promise((resolve, reject) => {
    ...
    sub.on("event", (event: Event) => {
      const stallEvt = {
        ...event,
        content: JSON.parse(event.content) as StallContent,
      } as StallEvent;
      onNew?.(stallEvt);
...
export async function getImageForStall(i: {
  stallId: string;
  author: string;
}): Promise<string> {
  const pool = new SimplePool();
...
export function groupShippinZones(
  stall: StallEvent,
...
export function removeItemFromCart(productId: string) {
  const cart = JSON.parse(
    localStorage.getItem("bitshop-cart") ?? "{products: [], meta: {}}"
  ) as BitshopCart;
