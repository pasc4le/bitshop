...
import { BitshopCart, BitshopCartProduct, ProductEvent, ShippingZone, StallEvent, getMerchantNameByPubkey, getProductById, getStallById, groupShippinZones, isBitshopCart } from './lib/nip15';
...
function Details() {
    ...
    useEffect(() => {
        !product && getProductById(id).then((e) => {
            setProduct(e);
            return e;
        })
            .catch((err) => {
        ...
        <>
            {
                props.images.map((e, idx) => {
                    return (
                        <div className={`image-wrap ${props.active === idx ? "active" : ""}`} onClick={() => props.setActive(idx)}>
...
function DetailsInner(props: { product: ProductEvent }) {
    ...
    const [shippingZones, setShippingZones] = useState<null | ShippingZone[]>(null)
    ...
    const addToCart = () => {
        const _qty = (document.querySelector("#input-quantity") as HTMLInputElement)?.value;
        ...
        let cart = JSON.parse(localStorage.getItem("bitshop-cart") ?? "[]") as BitshopCart;
                ...
                <ol className="breadcrumb">
                    <li className="breadcrumb-item"><a href="#">Home</a></li>
                    {
                        (() => {
                            ...
                            const tags = Array.from(Array.from(props.product.tags.filter(t => t[0] === "t")).entries());
                            return tags.map(([idx, value]) => <li className={`breadcrumb-item mid ${idx === tags.length - 1 ? "active" : ""}`}>
                                {
                                    idx === tags.length - 1 ? value[1] : <a href="#">{value[1]}</a>
                                }</li>
                            )
                        })()
            ...
            <div className='row'>
                    ...
                    <div className='product-img-wrap'>
                        <div className='row'>
                            <div className='col-12 col-md-2 nav-img'>
                                <DetailsImages active={active} setActive={setActive} images={props.product.content.images} />
                                {
                                    /*
                            ...
                            <div className='col-12 col-md-10 text-center'>
                                <img src={props.product.content.images[active]} style={{ maxWidth: "100%", "height": "auto", "padding": "2rem" }} alt='Products' />
                            </div>
                        </div>
                    </div>
                </div>
                <div className='col-12 col-md-5'>
                    ...
                    <span>
                        ...
                        <a href={href} style={{ "marginLeft": "0.5em" }}><strong>{getMerchantNameByPubkey(props.product.pubkey)}</strong></a>
                    </span>
                    { /*
                    <span>
                    ...
                    */
                    ...
                    <h6 className='mt-2'><strong>{props.product.content.price} USD</strong></h6>
                    {
                        /*
                    ...
                    <div className='mt-2 cart-wrap'>
                        <input id="input-quantity" type='number' max={props.product.content.quantity ?? 0} min={1} className='form-control' placeholder='Qty: 1' />
                        <button className='btn btn-theme' onClick={() => addToCart()}>Add to Cart</button>
                    </div>
                    <p className='mt-2'>Availability: <span className='text-success'>{props.product.content.quantity ?? "?"} in stock</span></p>
                    <p className='mt-2 mb-1'>Delivery:</p>
                    <div>
                        {
                            shippingZones && shippingZones.map((e) => {
                                ...
                                return (
                                    <div>
                                        ...
                                        />
                                        <label>Ship to {e["name"]} for {e["cost"]} USD</label>
                                    </div>
                                )
                            })
                    ...
                    <div className='details-tab'>
                        ...
                        >
                            <Tab eventKey="partnersShop" title="Description">
                                <div>
                                    {
                                        (props.product.content.description ?? "No description provided by seller").split("\n").map((e) => <p>{e}</p>)
                                    }
                                </div>
                            </Tab>
